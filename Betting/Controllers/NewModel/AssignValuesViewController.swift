...
class AssignValuesViewController: UIViewController {
    ...
    func setStatsChosen() {
        var modelParams = modelMaster.getUserModelParameters()
        
        for label in sliderLabelPair.values{
            modelParams.updateValue(label.text!, forKey: league.stats[label.tag])
        }
        modelMaster.setUserModelParameters(params: modelParams)
    }
    ...
    func populateStats() {
        let constant = (CGFloat)(modelMaster.getUserModelParameters().count * 100)
        ContentViewHeight.constant = constant
        
        let modelParams = modelMaster.getUserModelParameters()
        
        
        for stat in modelParams.keys {
            ...
            percentLabel.tag = league.stats.firstIndex(of: stat)!
